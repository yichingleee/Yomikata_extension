...
var globalObject = typeof globalThis !== "undefined"
  ...
  ? global
  : {};
globalObject.regeneratorRuntime = runtime;
...
var toRawRomaji = function toRawRomaji(str, system) {
  ...
  var romajiSystem = {
    nippon: {
      ...
      ?™„É•: "ryu",
      ?™„Éß: "ryo",
      // ?óÈü≥-ÊøÅÈü≥(?Æ„É£ÔΩû„????ÅÂ?ÊøÅÈü≥(?î„É£ÔΩû„????ÅÂ??óÈü≥(?Ø„ÉÆ?Å„Ç∞??
      ?é„?: "gya",
      ?é„?: "gyu",
      ?é„?: "gyo",
      ?ò„?: "zya",
  ...
  var ch;
  var r;
  var result = ""; // [PASSPORT] ?∑Èü≥?ÅÁï• ?å‚Äï„Äç„ÅÆ?¥Â?

  if (system === ROMANIZATION_SYSTEM.PASSPORT) {
    str = str.replace(/??gm, "");
  } // [NIPPON|HEPBURN] ?•Èü≥?ÆÁâπÊÆäË°®Ë®?a?Åi?Åu?Åe?Åo?Åy


  if (system === ROMANIZATION_SYSTEM.NIPPON || system === ROMANIZATION_SYSTEM.HEPBURN) {
    var reg_hatu = new RegExp(/(?ì|??(?=?Ç|?Ñ|?Ü|?à|?ä|?¢|?§|?¶|?®|?™|?Å|?É|?Ö|?á|?â|?°|?£|?•|?ß|?©|?Ñ|?Ü|?à|?§|?¶|?®|?É|?Ö|?á|?£|?•|??/g);
    var match;
    var indices = [];
  ...
  result = result.replace(reg_tsu, "$2$2"); // [PASSPORT|HEPBURN] Â≠êÈü≥?íÈ??≠„Å¶?πÊ?Ë°®Ë?

  if (system === ROMANIZATION_SYSTEM.PASSPORT || system === ROMANIZATION_SYSTEM.HEPBURN) {
    result = result.replace(/cc/gm, "tc");
  }

  result = result.replace(reg_xtsu, "tsu"); // [PASSPORT|HEPBURN] ?•Èü≥?ÆÁâπÊÆäË°®Ë®?b?Åm?Åp
  ...
  if (system === ROMANIZATION_SYSTEM.NIPPON) {
    ...
    result = result.replace(/i??gm, "√Æ");
    result = result.replace(/u??gm, "√ª");
...
var patchTokens = function patchTokens(tokens) {
  ...
  } // patch for ?©Â?Ë©??? after ?ïË?


  for (var i = 0; i < tokens.length; i++) {
    if (tokens[i].pos && tokens[i].pos === "?©Â?Ë©? && (tokens[i].surface_form === "?? || tokens[i].surface_form === "??)) {
      if (i - 1 >= 0 && tokens[i - 1].pos && tokens[i - 1].pos === "?ïË?") {
  ...
  } // patch for "?? at the tail of ?ïË??ÅÂΩ¢ÂÆπË?


  for (var j = 0; j < tokens.length; j++) {
    if (tokens[j].pos && (tokens[j].pos === "?ïË?" || tokens[j].pos === "ÂΩ¢ÂÆπË©?) && tokens[j].surface_form.length > 1 && (tokens[j].surface_form[tokens[j].surface_form.length - 1] === "?? || tokens[j].surface_form[tokens[j].surface_form.length - 1] === "??)) {
...
export default globalThis.Kuroshiro;
